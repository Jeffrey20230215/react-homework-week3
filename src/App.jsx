...
function App() {
  ...
  const proudctModalRef = useRef(null);
  const delProudctModalRef = useRef(null);
  const [modalMode, setModalMode] = useState(null);

  useEffect(() => {    
    new Modal(proudctModalRef.current, {
      backdrop: "static"
    });
    new Modal(delProudctModalRef.current, {
      backdrop: "static"
    });
  },[])
  ...
  const handleOpenProductModal = (mode, product) => {
    ...
    const modalInstance = Modal.getInstance(proudctModalRef.current);
    modalInstance.show();
  }
  ...
  const handleOpenDelProductModal = (product) => {
    setTempProduct(product);
    const modalInstance = Modal.getInstance(delProudctModalRef.current);
    modalInstance.show();
  }
  ...
  const updateProduct = async () => {
    try {
      await axios.put(`${API_BASE}/v2/api/${API_PATH}/admin/product/${tempProduct.id}`, {
        data: {
          ...tempProduct,
          origin_price: Number(tempProduct.origin_price),
          price: Number(tempProduct.price),
  ...
  const deleteProduct = async () => {
    try {
      await axios.delete(`${API_BASE}/v2/api/${API_PATH}/admin/product/${tempProduct.id}`);
    } catch (err) {
        alert('新增產品失敗');
    }
  }
  ...
  return (
    ...
    <div ref={proudctModalRef} id="productModal" className="modal" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
